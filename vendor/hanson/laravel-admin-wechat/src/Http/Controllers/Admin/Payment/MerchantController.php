...
class MerchantController extends AdminController
{
    ...
    protected function form()
    {
        ...
        $form->password('key', '秘钥')->required();
        //$form->text('notify_url', '回调地址');
        $form->file('cert_path', '微信支付证书')->required();
        $form->file('key_path', '微信支付证书密钥')->required();

        $form->saved(function (Form $form) {
            Cache::forever('wechat.merchant.mch_id.'.$form->model()->mch_id, [
                'app_id' => $form->model()->app_id,
                'mch_id' => $form->model()->mch_id,
                'key' => $form->model()->key,
                'notify_url' => $form->model()->notify_url,
                'cert_path' => $form->model()->cert_path,
                'key_path' => $form->model()->key_path,
            ]);
        });
