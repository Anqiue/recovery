(function () {
    ...
    const serviceHost = '/admin/upload/get_service_data';
    ...
    SKU.prototype.init = function () {
        ...
        _this.warp.find('.sku_edit_warp tbody').on('change', 'select', _this.processSku.bind(_this));
    ...
    SKU.prototype.SKUForm = function (default_sku) {
        ...
        if (attr_names.length === 0) {
            ...
            thead_html += '<th style="display:none;">id</th>';
            ...
            thead_html += '<th style="width: 100px">关联服务包</th>';
            thead_html += '</tr>';
            ...
            // 根据计算的笛卡尔积渲染tbody
            let tbody_html = '';
            cartesianProductOf.forEach(function (sku_item,key) {
                tbody_html += '<tr>';
                sku_item.forEach(function (attr_val, index) {
                    let attr_name = attr_names[index];
                    tbody_html += '<td data-field="' + attr_name + '">' + attr_val + '</td>';
                });
                tbody_html += '<td data-field="id" style="display:none;"><input value="' + (key+1) + '" type="text" class="form-control"></td>';
                ...
                tbody_html += '<td data-field="asso_service_id"><select name="service" id="select' + key + '" class="form-control">';
                tbody_html += '<option  value="0">无服务</option>';
                _this.selectService.forEach(function (service, index) {
                    tbody_html += '<option  value="' + service.id + '">' + service.product_name + '</option>';
                });
                tbody_html += '</select></td>';
            ...
            if(default_sku) {
                ...
                default_sku.forEach(function(item_sku, index) {
                    ...
                    Object.keys(item_sku).forEach(function(field) {
                        ...
                        if(field == 'asso_service_id'){
                            let select = tr.find('td[data-field="'+field+'"] select');
                            let id = select.attr('id');
                            $('#'+id).val(item_sku[field])
                        }
                    })
                });
            }
    ...
    SKU.prototype.getServiceData = function () {
        let _this = this;
        $.ajax({
            type: "POST",
            url: serviceHost,
        ...
        if (sku_json.type === 'many') {
            ...
            _this.warp.find('.sku_edit_warp tbody tr').each(function () {
                ...
                tr.find('td[data-field]').each(function () {
                    ...
                    }else if(field == 'asso_service_id'){
                        let select = td.find('select');
                        let id = select.attr('id');
                        item_sku[field] = $('#'+id).val();
                    } else {
